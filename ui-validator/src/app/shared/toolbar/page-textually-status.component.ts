  ...
  template: `
    <label [hidden]='loading' class='col-form-label'>
      ...
      <span *ngIf='totalItems > 0'> 
        {{firstItem + 1}} {{'TO' | translate}} {{lastItem}} {{'OF' | translate}} {{totalItems}}
      </span>
      {{'ENTRIES' | translate}}
      <span *ngIf='totalItems != absolutItems'>
        {{'Roas.FILTERED_FROM' | translate}} {{absolutItems}} {{'Roas.TOTAL_ENTRIES' | translate}}
      </span>
    </label>
  `
})
